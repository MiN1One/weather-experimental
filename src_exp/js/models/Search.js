...
export default class Search {
    ...
    async getResults() {
        try {
            const wb = await axios(`https://api.weatherbit.io/v2.0/current?key=${key.wbKey}&lang=${this.lang}&units=${this.metric}&city=${this.location}`);
            this.data = wb.data.data[0];
            this.all = wb.data.data;
            this.weather = this.data.weather.icon;
    ...
    async getForecast() {
        try {
            const wbF = await axios(`https:api.weatherbit.io/v2.0/forecast/daily?key=${key.wbKey}&lang=${this.lang}&units=${this.metric}&city=${this.location}`);
            this.forecast = wbF.data.data;
